...
public class SmsNurigoServiceImpl implements SmsNurigoService{
     ...
     * @param uuid 결제내역 PK
     */
    public void receipt (String toPhone, String uuid) {
        // localdatetime  읽기 편하게 변환
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

        // ReservationDetails 들고오기
        ReservationDetailsVO reservationDetailsVO = reservationDetailsMapper.selectOneById(uuid);
...
//        paymentDetailsDTO.getReservation().getSchedule().getMovie().getTitle()
        ...
        String content = "상영관: %s관\n인원수: %d\n좌석: %s\n영화시작시간: %s\n영회종료시간: %s\n등급: %s\n제목: %s\n결제금액: %d원\n결제시간: %s\n예매번호: %s\n결제수단: %s\n"
                .formatted(theaterNo, personnel, seatName, startAt, endAt, rating.getConversion(), title, cost, createAt, uuid, status);
